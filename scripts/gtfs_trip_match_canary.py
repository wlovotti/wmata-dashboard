...
date's `runs` rows (the failure shape the SFMTA 8/9–8/10 runs top-up
fix addressed, PR #TODO — a near-zero derive still writes `runs`
rows, which blocks `run_daily_batch.py`'s auto-revisit). See
