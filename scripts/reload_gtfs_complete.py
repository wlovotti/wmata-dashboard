...
from src.models import (Agency, Calendar, CalendarDate, FeedInfo, Timepoint, TimepointTime,
...
def reload_complete_gtfs():
    ...
    try:
        ...
        files_to_parse = [
            ...
            'routes.txt', 'stops.txt', 'trips.txt', 'stop_times.txt', 'shapes.txt',
            'timepoints.txt', 'timepoint_times.txt'
        ]
        ...
        try:
            ...
            for i, st_data in enumerate(gtfs_data['stop_times']):
                stop_time = StopTime(
                    ...
                    drop_off_type=int(st_data['drop_off_type']) if st_data.get('drop_off_type') else None,
                    shape_dist_traveled=float(st_data['shape_dist_traveled']) if st_data.get('shape_dist_traveled') else None,
            ...
            for i, tpt_data in enumerate(gtfs_data['timepoint_times']):
                timepoint_time = TimepointTime(
                    ...
                    drop_off_type=int(tpt_data['drop_off_type']) if tpt_data.get('drop_off_type') else None,
                    shape_dist_traveled=float(tpt_data['shape_dist_traveled']) if tpt_data.get('shape_dist_traveled') else None,
