...
Last edited 2026-05-15. Closed NOTES-44 — marginal-bus EWT model —
in this PR. Added a `/api/marginal-ewt` endpoint and a
`/marginal-bus` page that ranks (route, period) cells by the
closed-form `period_minutes / (2 N (N + 1))` SWT reduction from
adding one trip — the operator's "where should my next dollar go?"
view. AWT impact intentionally deferred (placement-strategy
dependent); the UI surfaces modeling caveats above the table so
the absolute number isn't over-interpreted vs the ranking.
