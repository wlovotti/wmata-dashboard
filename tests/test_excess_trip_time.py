...
class TestPipelineRouteMetricsDailyColumns:
    """Pipeline-side wiring (NOTES-43): excess-trip-time fields land in the
    `route_metrics_daily` row written by the daily pipeline.

    Mirrors the system_metrics test pattern. Because the full
    `compute_metrics_batch` path needs Routes / Trips / VehiclePositions /
    StopTimes / Stops set up, the assertion here targets the integration
    surface — the dict the save loop assembles — by calling the per-row
    helper directly via the dict shape it produces. We validate the
    pipeline imports and uses the right keys so a column rename here
    surfaces in tests rather than at production-write time.
    """

    def test_pipeline_dict_carries_excess_trip_time_keys(self, db_session):
        """Smoke check: the daily pipeline pulls in compute_excess_trip_time_for_routes
        and the new RouteMetricsDaily columns exist with the names we expect.
        """
        from pipelines.compute_daily_metrics import compute_excess_trip_time_for_routes
        from src.models import RouteMetricsDaily

        # The four columns must be on the model so SQLAlchemy can write them.
        cols = {c.name for c in RouteMetricsDaily.__table__.columns}
        assert "excess_trip_time_pct" in cols
        assert "median_actual_trip_time_sec" in cols
        assert "median_scheduled_trip_time_sec" in cols
        assert "excess_trip_time_n_trips" in cols

        # And the pipeline imports the batch function under the expected name.
        assert callable(compute_excess_trip_time_for_routes)

    def test_pipeline_writes_excess_columns_for_route_with_runs(self, db_session):
        """End-to-end: seed runs, call the batch function the pipeline calls,
        write the resulting fields onto a `RouteMetricsDaily` row, and read
        them back. Mirrors what the save loop in `compute_metrics_batch`
        does.
        """
        from src.models import RouteMetricsDaily
        from src.timezones import utcnow_naive

        # Seed two joined trips so the metric has signal: T1 on-time, T2 +400s
        # over the 3600s scheduled (just over the 110% threshold).
        for trip_id, extra_sec in [("PIPE_T1", 0), ("PIPE_T2", 400)]:
            db_session.add_all(
                [
                    _make_run(
                        trip_id,
                        "proximity",
                        first_obs_ts=SCHED_FIRST,
                        last_obs_ts=SCHED_FIRST + timedelta(minutes=30),
                        origin_dev_sec=0,
                        destination_dev_sec=None,
                        route_id="PIPE_R",
                    ),
                    _make_run(
                        trip_id,
                        "trip_update",
                        first_obs_ts=SCHED_FIRST + timedelta(seconds=60),
                        last_obs_ts=SCHED_LAST + timedelta(seconds=extra_sec),
                        origin_dev_sec=None,
                        destination_dev_sec=extra_sec,
                        route_id="PIPE_R",
                    ),
                ]
            )
        ...
        # Replicates what `compute_metrics_batch` does for excess-trip-time:
        # batch over routes for the date, then inject the keys into the
        # per-route metrics dict that builds the RouteMetricsDaily row.
        excess_results_list = compute_excess_trip_time_for_routes(
            db_session, SERVICE_DATE, route_ids=["PIPE_R"]
        )
        excess_by_route = {r["route_id"]: r for r in excess_results_list}
        excess = excess_by_route["PIPE_R"]
        assert excess["n_trips"] == 2

        n = excess["n_trips"]
        row = RouteMetricsDaily(
            route_id="PIPE_R",
            date=SERVICE_DATE_STR,
            otp_percentage=80.0,
            total_arrivals=20,
            unique_vehicles=2,
            excess_trip_time_pct=excess["pct_over_110"] if n > 0 else None,
            median_actual_trip_time_sec=int(excess["median_actual_sec"])
            if n > 0 and excess["median_actual_sec"] is not None
            else None,
            median_scheduled_trip_time_sec=int(excess["median_scheduled_sec"])
            if n > 0 and excess["median_scheduled_sec"] is not None
            else None,
            excess_trip_time_n_trips=n,
        ...
        read_back = (
            ...
            .filter(
                RouteMetricsDaily.route_id == "PIPE_R",
                RouteMetricsDaily.date == SERVICE_DATE_STR,
            )
            .first()
        )
        assert read_back is not None
        # 1 of 2 trips over threshold = 50%
        assert read_back.excess_trip_time_pct == 50.0
        assert read_back.median_scheduled_trip_time_sec == 3600
        assert read_back.excess_trip_time_n_trips == 2
        # Median actual: median of [3600, 4000] = 3800
        assert read_back.median_actual_trip_time_sec == 3800

    def test_pipeline_route_with_no_runs_writes_null_excess_fields(self, db_session):
        """Routes with no qualifying trips get None on the excess columns —
        the save loop's `n_excess_trips > 0` guard guarantees this so the
        UI null-handling rules apply (sparkline/card suppress on null).
        """
        from src.models import RouteMetricsDaily
        from src.timezones import utcnow_naive

        excess_results_list = compute_excess_trip_time_for_routes(
            db_session, SERVICE_DATE, route_ids=["EMPTY_R"]
        )
        excess = {r["route_id"]: r for r in excess_results_list}["EMPTY_R"]
        assert excess["n_trips"] == 0
        assert excess["pct_over_110"] is None
        ...
        row = RouteMetricsDaily(
            ...
            unique_vehicles=1,
            excess_trip_time_pct=excess["pct_over_110"] if n > 0 else None,
            median_actual_trip_time_sec=None,
            median_scheduled_trip_time_sec=None,
            excess_trip_time_n_trips=int(n),
            computed_at=utcnow_naive(),
        )
        db_session.add(row)
        db_session.commit()
        ...
        read_back = (
            db_session.query(RouteMetricsDaily)
            .filter(
                RouteMetricsDaily.route_id == "EMPTY_R",
                RouteMetricsDaily.date == SERVICE_DATE_STR,
            )
            .first()
        )
        assert read_back is not None
        assert read_back.excess_trip_time_pct is None
        assert read_back.median_actual_trip_time_sec is None
        assert read_back.median_scheduled_trip_time_sec is None
        assert read_back.excess_trip_time_n_trips == 0
